# Foundations of Cybersecurity

## Overview  
This course gave me a solid foundation in cybersecurity. It covered important concepts that every junior security analyst should understand. In this section, I will briefly explain the key topics I learned throughout the course.

---

## Responsibilities of a Cyber security analyst

 A cyber security analyst is the person who monitors and protects an organization's systems, data and assets by identifying threats , finding vulnerabilities and helping reduce security risks.
 
Their responsibilities consists of :

* monitoring networks and systems
* investigating security incidents
* responding to incidents
* identifying vulnerabilities
* manage and recommend security controls
* analyze logs
* perform risk assessments
*  Help protect CIA triad
* improve security practices
* document incidents and findings

   For an entry level security analysts they are not required to do all of this since they will gain experience as they are working.
   But most importantly they should be able to monitor systems , protect information and search for weaknesses.
---

  ## Ethics in Cyber Security

We already know that cybersecurity professionals have privileges like accessing sensitive data. They have to respect that privilege; it is crucial to adhere to established policies and protocols. Security professionals must also remain unbiased and uphold security and confidentiality even when personal relationships are involved.

#### Key ethical Principles

**Confidentiality**: professionals have an ethical duty to keep private information , such as PII and SPII confidential and secure. 

**Privacy protections**: Safeguarding personal information from unauthorized use is essential , and professionals should always follow company policies regarding data access and sharing. 

#### laws and compliance considerations 

security professionals must understand and comply with laws and regulations that govern data handling and security. negligence in following compliance regulations can lead to serious consequences , including legal repercussions and loss of data. 

---

### Deliberate PII handling 

PII is personally identifiable information, so organizations must be deliberate about how PII or SPII is handled , where it is stored and consistently aware of iits storage location.

**Consequences of Non-Compliance**

* Failure to meet PII obligations can lead to government regulators inspecting a company's data handling practices.
* Customers and businesses may directly inquire about data handling, impacting customer relationships, especially with sensitive data.

---

## Types of attacks

### Physical attacks 

A security incident that affects digital and physical environments where the incident is deployed 

* Malicious USB cable
* Malicious flash drive
* card cloning and kimming 

### Supply-chain attack

A **supply-chain attack** targets systems, applications, hardware, and/or software to locate a vulnerability where malware can be deployed.These attacks are costly because they can affect multiple organizations and the individuals who work for them.


### Cryptographic attacks

**cryptographic attack** affects secure forms of communication between a sender and intended recipient. Some forms of cryptographic attacks are: 

* Birthday
* Collision
* Downgrade
   

### Password attack

A **password attack** is an attempt to access password-secured devices, systems, networks, or data. 

* Brute force
* Rainbow table



## Social engineering





## Phishing 
  
   Phishing is the use of digital communications to trick people into revealing sensitive data of deploying malicious software Types of phishing Business email compromise (BEC) is when a threat actor sends an email message that seems to be from known source to make a seemingly a true request for information , in order to obtain a financial advantage. Spear phishing is a malicious email attack that targets a specific user or group of users. The email seems to originate from a trusted source. Whaling is also a form of spear phishing . It is when a threat actors targets company executives to gain access to sensitive data . Vishing its is the exploitation of electronic voice communication to obtain sensitive information or impersonate a known source. Smishing is the use of text messages to trick users in order to obtain sensitive information.



















  
